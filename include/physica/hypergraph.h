...
class node : public internal::identifiable_base
{
 ...
 public:
   ...
   bool operator==(const node &rhs) const { return internal::identifiable_base::operator==(rhs); }
 ...
 protected:
   ...
   {
      if (!is_incident_to(incidentEdge))
      {
...
class hyperedge : internal::identifiable_base, std::enable_shared_from_this<hyperedge<T>>
{
 ...
 public:
   ...
   hyperedge(const std::vector<std::shared_ptr<node<T>>> &nodes) : internal::identifiable_base()
   {
 ...
 protected:
   const typename std::vector<std::shared_ptr<node<T>>>::iterator
   find_node(std::weak_ptr<node<T>> node)
   {
...
class hypergraph
{
 public:
   ...
   {
      nodes_.emplace_back(std::make_shared<node<T>>(std::forward<Args>(args)...));
      return std::weak_ptr<node<T>>(nodes_.back());
   }
   ...
   const std::weak_ptr<hyperedge<T>> add_edge(std::vector<std::weak_ptr<node<T>>> nodes)
   {
      ...
      // add the edge to the graph and give the incident vertices a ref to the
      // new edge
   ...
   void remove_edge(const std::weak_ptr<hyperedge<T>> &edge)
   {
      ...
      {
         (*edgeIter)->remove_from_node_incidence_lists();
         edges_.erase(edgeIter);
      }
   }

   const std::vector<std::weak_ptr<hyperedge<T>>>
   add_edges(std::vector<std::vector<std::weak_ptr<node<T>>>> nodeSets)
   {
